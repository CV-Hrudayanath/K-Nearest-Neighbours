function out = kfoldsampling( dataset , size , training_set_size ,k , no_of_attributes , no_of_classes, classifier_index,class_data)
...
for iter = 1:5
        position = [];
        start = (iter-1)*test_set_size + 1;
        ending = iter*test_set_size;
        training_set = [];
        for i = 1:start
            p1 = [];
            position = [ position i];
        ...
        for i = 1:test_set_size
            ...
            actual = class_no(class_data, data{classifier_index}(sampled_index((start-1)+i)), no_of_classes);
            %disp(actual);
            confusion_matrix(actual,predicted) = confusion_matrix(actual,predicted) + 1; 
            if strcmp( data{classifier_index}(sampled_index(selected_index(i))) , data{classifier_index}(sampled_index((start-1)+i)) ) == 1
                count = count + 1;
            end  
        ...
        fprintf('Accuracy of Fold %d is %f\n',iter , accuracy);
